...
const searchForTicker = (messageContent: string) => {
  ...
  const NYSETickers = NYSESymbolList.filter((symbol) =>
    containsSymbol(tokenizedMessage, symbol)
  ).map((symbol) => getTicker('NYSE', symbol));

  const NASDAQTickers = NASDAQSymbolList.filter((symbol) =>
    containsSymbol(tokenizedMessage, symbol)
  ).map((symbol) => getTicker('NASDAQ', symbol));

  const AMEXTickers = AMEXSymbolList.filter((symbol) =>
    containsSymbol(tokenizedMessage, symbol)
  ).map((symbol) => getTicker('AMEX', symbol));
...
const getTickers = (messageContent: string, operation: string): Ticker[] => {
  ...
  if (operation !== 'N/A') {
    ...
    if (foundTickers.length > 1) {
      foundTickers = foundTickers.filter(
        (ticker) => !ticker.symbol.includes('-')
      );
...
const getExpirationDate = (tokenizedMessage: string[]) => {
  const expirationDate =
    tokenizedMessage.filter((str) => str.includes('/'))[0] ||
    tokenizedMessage[1];
...
const getOrderType = (messageContent: string, parsedStrike: string) => {
  let type = 'N/A';
  type =
    messageContent.includes('call') || messageContent.includes('CALL')
      ? 'CALL'
      : type;
  type =
    messageContent.includes('put') || messageContent.includes('PUT')
      ? 'PUT'
      : type;
  if (type === 'N/A') {
...
const getOrderTarget = (messageContent: string, strikePrice: string) => {
  ...
  if (messageContent.includes('target') || messageContent.includes('Target')) {
    const targetSplits =
      messageContent.split('target') || messageContent.split('Target');
...
export const parseTrade = (
  ...
  try {
    if (operation === 'STC') {
      ...
      const tokenizedMessage = message.content
        .toUpperCase()
        .split(tickers[0].symbol)[1]
        .split(' ');
      ...
      const target = getOrderTarget(message.content, parsedStrike).filter(
        (t) => parseInt(t) > 0
      );
